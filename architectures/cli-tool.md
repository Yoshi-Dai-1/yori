...
具体的な言語・パッケージマネージャーが確定したら、`stack-setup.md` の Step 3.5 ブロックB が
必須ファイルを自律検索して補完する。
...
登録方法は言語・ツールによって異なるため、`stack-setup.md` の Step 3.5 ブロックB が自律検索する。
...
各チャネルの最新の登録手順は年々変わるため、採用時にブロックCが自律検索する。
