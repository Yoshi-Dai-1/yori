...
`tool.execute.before` で bash ツールのコマンド文字列に `git commit` が含まれていることを検出して発火する（`git -C <dir> commit` / `git -c key=val commit` 等の `--flag` 前置形式も検出）。
...
- **ブロック**: ブロック対象クラス（機密情報のハードコード・認証/認可の欠如・インジェクション・機密情報のログ出力）または `[重要度: HIGH/CRITICAL]` に該当し、**「検証方法」が添えられた**指摘（`evidenceRequired: true` のため、検証方法がない場合は HIGH/CRITICAL でもブロックしない・警告化）
- **指摘マーカーの認識契約**: `[重要度: X]` は**行頭（bullet/太字前置を除く）でのみ認識**する。監査報告の引用・結論文・まとめ等の文中に現れても指摘には数えない（幽霊指摘の発生源を塞ぐ）。監査エージェント側にも同じ契約をプロンプトで明記している
...
- 【残存確認】は履歴の元の指摘（同じファイル）の重要度・クラスを引き継ぎ、ブロック条件は通常の判定と同一
  （`block.severities` またはブロック対象クラス → ブロック、それ以外は警告記録。履歴に見つからない場合は安全側で HIGH）
- 行番号ドリフト許容は**「そのファイルに該当レコードが 1 件だけ」の場合に限定**する。同ファイルに複数レコードがある場合は行一致を要求し、別行・別指摘の取り違え（fail-open）を防ぐ
- 履歴へ書き出す指摘本文・検証エビデンスは `secret-patterns.json` の block 系パターン + 鍵本文/Bearer/JWT/長い不透明トークンの補完パターンで**マスク**され、秘密値の実体が `docs/review-log.md`（git 履歴）に永続化しない
- 生成される `docs/review-log.md` は**生成状態ファイル**であり prettier の整形対象外（CI で `prettier --check .` を回すプロジェクトは `.prettierignore` に追加する）。書き出し時は可読性のためリスト行の連続スペースを 1 つに折り畳む
