...
- スキルGCレポート：`.opencode/usage/skill-usage.md` を読み、各スキルの出現回数と
  最終使用日を集計して報告する（使用回数＝同名の出現回数・最終使用日＝最後の記録日付）。
  対象月3回以上は積極使用、1〜2回は普通、`.opencode/skills/` に存在するが対象月0回は
  削除候補として提案する。対象月はレポート見出しの YYYY-MM（未指定なら診断実行月の前月）とする。find-skills・skill-creator はグローバルスキルのため除外する。
