...
審査系サブエージェント（code-reviewer / security-auditor）をゲートとして使うときの原則。

1. **読み取り専用**: 審査するだけで修正しない。書き込み・変更はゲート対象外の機構が担う
2. **指摘は検証可能な形で報告する**: ブロック対象の指摘には「検証方法」（grep / CLI コマンド / 監査出力の該当行）を必須とする。感覚的な懸念で門を止めない
3. **ゲートは状態を持つ**: 審査結果を履歴（`docs/review-log.md`）に記録し、次回の審査に引き継ぐ。同一指摘の再報告・解消済み指摘の再ブロックを防ぐ
4. **判定はクラスで行う**: ブロック対象は主観 severity だけに頼らず、クラス（機密のハードコード・認証/認可の欠如・インジェクション等）で判定する。severity の語彙・閾値の SSoT は `.opencode/config/review-policy.json`
5. **配役を一意化する**: オーケストレータ（commit-review 等）が配役を定め、エージェント同士の委譲で同じ仕事を二重に実行させない
