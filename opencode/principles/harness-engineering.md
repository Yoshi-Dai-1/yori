...
並列子セッションで実行し、判定ポリシーに従ってブロックする。依存マニフェストが diff に含まれる場合は
検出した全てに対して npm audit / pip-audit 等の決定的な依存監査も付加する。オーケストレータ役であり、サブエージェント同士の
委譲は行わない（二重監査を防ぐ）。
...
- 【残存確認】は履歴の元の指摘（同じファイル）の重要度・クラスを引き継ぎ、ブロック条件は通常の判定と同一
  （`block.severities` またはブロック対象クラス → ブロック、それ以外は警告記録）。履歴に見つからない場合は安全側で HIGH（ブロック）
- 行番号ドリフト許容は**「そのファイルに該当レコードが 1 件だけ」の場合に限定**する。同ファイルに複数レコードがある場合は
  行一致を要求し、別行・別指摘の取り違え（fail-open）を防ぐ
- 指摘マーカー（`[重要度: X]`）は**行頭（bullet/太字前置を除く）でのみ認識**する。監査報告の引用や結論文の文中に
  現れても指摘として数えない（幽霊指摘の発生源を塞ぐ）。監査エージェント側のプロンプトにも同じ契約を明記する
- 審査履歴へ書き出す指摘本文・検証エビデンスは `secret-patterns.json` の block 系パターン + 鍵本文/Bearer/JWT/
  長い不透明トークンの補完パターンで**マスク**して記録し、秘密値の実体が git 履歴に永続化する二次経路（secret-in-log）を塞ぐ
- 生成される review-log.md は**生成状態ファイル**であり prettier の整形対象外（CI で `prettier --check .` を回す
  プロジェクトは `.prettierignore` に追加する）。書き出し時は可読性のためリスト行の連続スペースを 1 つに折り畳むだけにする
...
- 両方の審査エージェントが起動されたのに無言で結果を返さない場合も**fail-closed でブロック**し、審査未成立を素通ししない
