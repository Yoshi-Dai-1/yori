...
if [ "$HAS_UI_FLAG" = true ]; then
  ...
  if [[ "$USE_OPEN_DESIGN" =~ ^[Yy]$ ]]; then
    ...
    echo "   この案内は .claude/standards/principles/design-contract.md の「Open Design との関係」セクションにも記載されています。"
    echo ""
    echo "✅ Open Design のセットアップ案内を表示しました（後から .claude/standards/principles/design-contract.md を参照）"
  else
