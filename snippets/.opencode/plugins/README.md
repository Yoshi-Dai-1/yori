...
| `lint-and-typecheck.ts` | `tool.execute.after` | ファイル編集後の lint・format・typecheck（P1-2 修正：性能改善） |
...
| Python | `ruff format` | `ruff check` | `mypy --follow-imports=silent --no-incremental ${fp}`（P1-2 修正：ファイル単位） |
