...
| トリガー | 参照先 |
|---------|--------|
| project-definition.md 作成・更新時 | `.opencode/rules/security/_trigger-project-definition.md` |
| 最初のコードファイル作成時 | `.opencode/rules/security/_trigger-first-code.md` |
| スプリント完了後（セキュリティ監査） | `.opencode/rules/security/_trigger-sprint-audit.md` |
| PR作成時（gh pr create / git push） | `.opencode/rules/security/_trigger-pr.md` |
| 新機能実装時（Won't違反チェック） | `.opencode/rules/security/_trigger-wont.md` |
| package.json バージョン変更時 | `.opencode/rules/security/_trigger-package-version.md` |
...
## リスク深刻度と対処タイミング
→ `.opencode/rules/security/_risk-severity.md` を読む

## Web検索の実行タイミングと情報源（言語別コマンド対応表を含む）
→ `.opencode/rules/security/_web-search.md` を読む
