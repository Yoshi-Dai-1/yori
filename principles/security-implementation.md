...
.opencode/agents/subagents/security-auditor.md        → 実装前に何を決めるか（選択肢の提示・対話）
.opencode/rules/security.md          → 常駐ルール・禁止事項・自律トリガー
...
```
参照タイミングと参照元：
  セキュリティ関連機能の実装依頼を受けたとき → .opencode/rules/security.md の自律トリガー経由
  ARCHITECTURE.md Step3 完了後              → ARCHITECTURE.md.template に組み込み済み
  @security-auditor 設計モード完了後         → .opencode/agents/subagents/security-auditor.md の実装手順
```
...
原則2：CSRF対策
  → SameSite Cookie属性を設定する
    ...
    SameSite属性をStrictにするかLaxにするか判断できない場合：Strictをデフォルトとし、.opencode/agents/subagents/security-auditor.md の設計モードで確認する
...
Step 2：.opencode/agents/subagents/security-auditor.md の設計モードで実装前の決定事項を確認する
...
2. 発見したリスクをリスク深刻度（CRITICAL/HIGH/MEDIUM/LOW）で分類して報告する
   → 分類基準は .opencode/rules/security.md の「リスク深刻度と対処タイミング」を参照
...
```
Step 1：依存ライブラリのスキャン
  → .opencode/rules/security.md の言語別コマンド対応表に従いauditを実行する
...
Lv.2以下のプロジェクトでCIがない場合：
  ...
  → 代替として依存ライブラリスキャン（.opencode/rules/security.mdの対応表）を
