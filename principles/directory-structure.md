...
目安：file-size-and-cohesion.md の閾値（300行）を参照。超えたら責務を分割する。
...
5. 1ファイルが file-size-and-cohesion.md の閾値を超えていないか？           → 分割を検討
...
### 必須ファイルの確認手順（`stack-setup.md` Step 3.5 ブロックB（必須ファイルの確認・記録）から呼び出される）

このセクションは `stack-setup.md` の Step 3.5 ブロックB（必須ファイルの確認・記録）が呼び出したときのみ実行する。
