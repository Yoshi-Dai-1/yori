...
フレームワーク固有のルーティング記法はブロックCが自律検索して補完する。
...
フレームワークが強制するルーティング記法はブロックCが自律検索して確認する。
...
スキーマの詳細設計（フラグメント・ディレクティブ等）はブロックCが自律検索して補完する。
...
フレームワーク固有の実装（Socket.io等）はブロックCが自律検索して補完する。
...
DBスキーマの設計原則は変化しない。マイグレーション戦略の実装詳細はブロックCが自律検索して補完する。
...
ブロックCが自律検索して確認する：`[ツール名] migration best practices [現在年]`
...
## 命名規則の確定手順（`stack-setup.md` Step 3.5 ブロックAから呼び出される）

このセクションは `stack-setup.md` の Step 3.5 ブロックAが呼び出したときのみ実行する。
...
3. 使用フレームワークの公式ドキュメントのコーディング規約セクション

URLは変更される可能性があるため、ドメイン名・組織名で情報源を判断する。
検索結果が複数ある場合は優先順位1のものを採用し、見つからない場合は順位を下げて探す。

**Step 2：検索結果を📋フォーマットで人間に通知する**
...
参照情報源（📋）も同セクションの末尾に記録する。これが SSOT となる。
