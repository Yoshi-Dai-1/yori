...
`.claude/rules/security.md` の「ゼロデイ・多層防御」セクションに基づき、
...
プロジェクト規模 × データ感度 → 対応レベル
             ...
             │ 機密なし │ 個人情報 │ 決済・医療
─────────────┼─────────┼─────────┼──────────
個人・小規模  │  Lv.1   │  Lv.2   │   Lv.3
中規模チーム  │  Lv.2   │  Lv.3   │   Lv.4
大規模・商用  │  Lv.3   │  Lv.4   │  Lv.4+
```
...
著作権・ライセンス → OSSライブラリを使用するすべてのプロジェクトに適用
  ...
  → ライセンス確認は `stack-setup.md` の Step 3.5 ブロックE が自律実行する
...
詳細は .claude/standards/principles/risk-based-approach.md の「リスクの4象限」を参照。
```
