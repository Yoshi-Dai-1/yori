...
著作権・ライセンス → OSSライブラリを使用するすべてのプロジェクトに適用
  ...
  → ライセンス確認は `.opencode/rules/stack-setup.md` の Step 3.5 ブロックE（ライセンス確認とプライバシーポリシー要否の判定）が自律実行する
...
| 実装依頼を受けたとき | `.opencode/agents/subagents/security-auditor.md` の設計モード |
