...
外部エンティティの特定：
  ...
  （project-definition.md に記載の対象ユーザーと連携サービスを参照して特定する）
```
...
```
CRITICAL（即時対処）：影響度 高 × 発生確率 高
HIGH（このスプリント内）：影響度 高 × 発生確率 低、または影響度 低 × 発生確率 高
MEDIUM（計画的に対処）：影響度 中 × 発生確率 中
LOW（記録して管理）：影響度 低 × 発生確率 低
```
