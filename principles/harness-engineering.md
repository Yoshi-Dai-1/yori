...
| 確認する問い | Noのとき何が起きるか |
|------------|-------------------|
| このルールは `AGENTS.md` か `.claude/rules/` に書いてあるか | エージェントはルールがないものとして動く |
...
| 即時フィードバック | lint をファイル編集後に自動実行してAIへ返す | `on-post-tool-use.lint-and-typecheck.sh` Hook |
...
両方を `on-post-tool-use.lint-and-typecheck.sh` Hook に組み込み、ファイル編集のたびに自動実行する。
...
使用言語の設定例は `.claude/standards/architectures/` の各アーキテクチャファイルを参照。
...
導入したい場合は `.claude/hooks/README.md` を参照して、AIと相談しながら判断する。
