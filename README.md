...
- `opencode.json`（プロジェクトルート）にテンプレート設定をコピー済み
- `.opencode/rules/*.md` を自動ロードする `instructions[]` 設定が有効
- 品質診断戦略を選択して `ARCHITECTURE.md` に記録済み
- `docs/working/` に作業ディレクトリテンプレートをコピー済み（plan.md / notes.md / review-checklist.md）

**Active Plugins：**
`.opencode/plugins/` に11つの TS Plugin が配置されています。
...
rules/の育て方：同じ指摘をAIから2回受けたとき → 「これをrulesに追加して」と依頼する。
           AIがルールファイルを作成して .opencode/rules/ に保存する。
